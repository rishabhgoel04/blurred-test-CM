query="""select oi.sku_id,  p.product_name_en, oi.product_image
from order_items oi
...
def get_data(query):
    ...
    conn = pg.connect(host="cmdb-rr.cbo3ijdmzhje.ap-south-1.rds.amazonaws.com",database="cmdb",port="5432",user="deepank_w",password="PNkzr44Yd7MZydGzS5q4HzMqBhV")
...
product_data_df=get_data(query)
...
# qualityscore = test_measure_BRISQUE(path)
...
for image_url in image_url_list:
    ...
    h=image.shape[0]
    w=image.shape[1]
    if(qualityscore<50.0) and (h<=1000) and (w<=1000):
